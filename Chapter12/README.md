...
- 여러 노드를 하나의 논리적 클러스터로 결합.
- 가용성에 영향을 주지 않고 동적으로 노드를 추가 및 제거
- 다운타임이 없는지 확인, 인스턴스가 중지되거나 상태 확인에 응답하지 않으면, 자동 재시작.
- 클러스터의 노드가 실패하면 해당 노드에서 실행중인 워크로드가 자동으로 다른 노드로 마이그레이션
- 서비스 검색 및 로드밸런싱
- 내구성이 있는 스토리지에 대한 오케스트레이션된 접근을 제공
- 다운타임 없는 자동 롤아웃 및 롤백
- 민감한 데이터 및 설정 관리 시스템을 위한 보안 저장공간

쿠버네티스 구성 개념은 '객체' 개념을 중심
객체의 예시: 컨테이너화 된 앱, 컨테이너 리소스, 앱 동작에 대한 정책
...
모놀리식이 나쁘다는건 오해다.  
종종 고도로 모듈화된 아키텍처와 내부 컴포넌트 간에 좋은 수준의 디커플링을 가지고 잇다.

완벽한 예로는 모놀리식 커널이라는 범주의 일부인 linuxOS 커널
os 전체가 다운되는 커널패닉이 있을 수 있음.
...
Y축 확장을 의의.
마이크로서비스를 채택하는데에.
서비스의 크기가 중요한 것은 아님. 느슨한 결합, 높은 응집력 및 통합 복잡성 등등.

서비스의 데이터 소유권을 위해 디비를 분할해야 한다.? (이상적인 msa의 궁극이라고 생각함.)

플랫폼 및 언어 간의 재사용성.

언어 뿐 아니라 서비스별 독립 디비를 갖게되면
서비스에 잘 맞는 데이터 저장소 선택(ex. 검색서버 - elasticsearch)

#### 마이크로서비스의 과제

Node.js의 모듈 시스템이 마이크로서비스와의 완벽한 동반자라고 소개.
...
원격 API 간에 통신을 프록시하는 서버를 사용하기(api 게이트웨이)
MSA 인프라 사용을 위한 단일 뷰를 제공.
하위 서비스들의 변경에 유리.

- API 오케스트레이션

프록시와 유사하나 데이터 집계에 용이하고, 다양한 서비스의 의미통합을 수행.  
기본 서비스에 의해 노출된 것과 다른 API 를 노출하기로 함.
